 ...
 */
public class MinSpanTree extends BaseJob {

	static enum MinSpanTreeCounters {
	 ...
	 * 
	 *        
     *         Description : MinSpanTreeMapper class that emits the input records in the form of <key, value> pairs where the key is the 		       weight and the value is the source, destination pair. MapReduce has automatic sorting by keys after the map phase.
	...
	public static class MinSpanTreeMapper extends Mapper<Object, Text, IntWritable, Text> {
	...
	/**
	 * MinSpanTreeReducer is based on Kruskal's algorithm. 
	 * 
	 *
	 *
	 * Kruskal's algorithm : the MinSpanTree is built as a forest. The edges are sorted by weights and considered in the increasing
	 ...
	 * disjoint trees , then they are merged and the edge is considered to be in the MinSpanTree. Else, the edge is discarded.
	 *
	 */

	//The reducer to form the minimum spanning tree based on Kruskal's algorithm
	static class MinSpanTreeReducer extends Reducer<IntWritable, Text, Text, Text> {
		...
		public void reduce(IntWritable inputKey, Iterable<Text> values, Context context)
			...
			for (Text val : values) {
				...
				if (!ignoreEdgeSameSet1 && !ignoreEdgeSameSet2 && !ignoreEdgeSameSet3) {
					...
					context.getCounter(MinSpanTreeCounters.totalWeight).increment(
							weight);
	...
	public int run(String[] args) throws Exception {
		...
		formMinSpanTreeJob(args[0], args[1]);

		return 0;

	}

	//method to run the job that forms the MinSpanTree
	private void formMinSpanTreeJob(String inputPath, String outputPath)
			throws Exception {
		Job MinSpanTreeJob = getMinSpanTreeJobConf(); //get the job configurations

		FileInputFormat.setInputPaths(MinSpanTreeJob, new Path(inputPath)); // setting the input files for the job
		FileOutputFormat.setOutputPath(MinSpanTreeJob, new Path(outputPath)); // setting the output files for the job

		MinSpanTreeJob.waitForCompletion(true);

		Counters jobCntrs = MinSpanTreeJob.getCounters();//get all the counters associated with MinSpanTreeJob

		long totalWeight = jobCntrs.findCounter(MinSpanTreeCounters.totalWeight)
				.getValue();
		
		System.out.println("The total weight of the MinSpanTree is " + totalWeight	);

	}

//get the job configuration  for formMinSpanTree mapper and reducer
	private Job getMinSpanTreeJobConf() throws Exception {
		...
		JobInfo jobInfo = new JobInfo() {
			...
			public Class<?> getJarByClass() {
				return MinSpanTree.class;
			}

			@Override
			public Class<? extends Mapper> getMapperClass() {
				return MinSpanTreeMapper.class;
			}
			...
			public Class<? extends Reducer> getReducerClass() {
				return MinSpanTreeReducer.class;
			}
		};
		
		return setupJob("formMinSpanTree", jobInfo);
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(new Configuration(), new MinSpanTree(), args);
		if (args.length != 2) {
			System.err
					.println("Usage: MinSpanTree <in> <output > ");
			System.exit(2);
		}
		System.exit(res);
	}
